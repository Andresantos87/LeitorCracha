import { Activity, Users, FileCheck2, BarChart3 } from "lucide-react";
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="bg-slate-800/50 rounded-xl p-6 border border-slate-700 shadow-sm backdrop-blur-sm">
          ...
          <p className="text-xs text-slate-400 mt-1">Sendo realizados hoje</p>
        </div>
        ...
        <div className="bg-slate-800/50 rounded-xl p-6 border border-slate-700 shadow-sm backdrop-blur-sm">
          ...
          <div className="text-2xl font-bold text-white mt-2">1,245</div>
          <p className="text-xs text-slate-400 mt-1">Importados via Excel</p>
        </div>

        {/* Card 3 */}
        <div className="bg-slate-800/50 rounded-xl p-6 border border-slate-700 shadow-sm backdrop-blur-sm">
          <div className="flex flex-row items-center justify-between space-y-0 pb-2">
            <h3 className="tracking-tight text-sm font-medium text-slate-300">Presenças Hoje</h3>
            <FileCheck2 className="h-4 w-4 text-indigo-400" />
          </div>
          <div className="text-2xl font-bold text-white mt-2">142</div>
          <p className="text-xs text-slate-400 mt-1">+12% em relação a ontem</p>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-7">
        <div className="col-span-4 bg-slate-800/50 rounded-xl border border-slate-700 p-6">
          <div className="flex flex-col space-y-1.5 mb-4">
            ...
            <p className="text-sm text-slate-400">Últimos treinamentos realizados nas plantas.</p>
          </div>
          <div className="space-y-4">
            {[
              { nome: "Integração de Segurança", data: "Hoje, 14:00", presencas: 45, planta: "Guaíba" },
              { nome: "Treinamento NR-10", data: "Hoje, 09:00", presencas: 12, planta: "Santa Fé" },
              { nome: "Operação de Empilhadeira", data: "Ontem, 16:30", presencas: 8, planta: "Guaíba" },
            ].map((t, i) => (
              <div key={i} className="flex items-center justify-between p-4 bg-slate-900/50 rounded-lg border border-slate-800">
                <div className="space-y-1">
                  <p className="text-sm font-medium leading-none">{t.nome}</p>
                  <p className="text-sm text-slate-400">{t.data} - {t.planta}</p>
                </div>
                <div className="flex items-center space-x-4">
                  <div className="text-sm font-medium text-emerald-400">{t.presencas} registrados</div>
                  <button className="text-blue-400 hover:text-blue-300 text-sm font-medium px-3 py-1 bg-blue-900/30 rounded-full">
                    Ver Detalhes
                  </button>
                </div>
              </div>
