      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="bg-slate-800/50 rounded-xl p-6 border border-slate-700 shadow-sm backdrop-blur-sm">
          <div className="flex flex-row items-center justify-between space-y-0 pb-2">
            <h3 className="tracking-tight text-sm font-medium text-slate-300">{t.activeTrainings}</h3>
        ...
        <div className="bg-slate-800/50 rounded-xl p-6 border border-slate-700 shadow-sm backdrop-blur-sm">
          <div className="flex flex-row items-center justify-between space-y-0 pb-2">
            <h3 className="tracking-tight text-sm font-medium text-slate-300">{t.adhesionRate}</h3>
            <BarChart3 className="h-4 w-4 text-purple-400" />
          </div>
          <div className="text-2xl font-bold text-white mt-2">--%</div>
          <p className="text-xs text-slate-400 mt-1">{t.avgTrainings}</p>
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-7">
        <div className="col-span-4 bg-slate-800/50 rounded-xl border border-slate-700 p-6">
          <div className="flex flex-col space-y-1.5 mb-4">
            <h3 className="font-semibold leading-none tracking-tight">{t.recentTrainings}</h3>
            <p className="text-sm text-slate-400">{t.latestRooms}</p>
          </div>
              ...
              treinamentos.map(t => (
                <div key={t.id} className="flex items-center justify-between p-4 bg-slate-900/50 rounded-lg border border-slate-800 hover:border-slate-700 transition-colors">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-[10px] text-slate-500 uppercase tracking-wider">{t.readStatus}</p>
                  </div>
                </div>
              ))
            )}
        ...
        <div className="col-span-3 bg-slate-800/50 rounded-xl border border-slate-700 p-6">
          <div className="flex flex-col space-y-1.5 mb-4">
            <h3 className="font-semibold leading-none tracking-tight">{t.quickActions}</h3>
            <p className="text-sm text-slate-400">{t.shortcuts}</p>
          </div>
